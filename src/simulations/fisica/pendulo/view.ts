...
export function penduloView(): HTMLElement {
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <p>Oscilación con longitud, gravedad y amortiguamiento ajustables.</p>
    </div>
  `;
  ...
  let theta = Math.PI/4; // ángulo inicial
  let omega = 0;
  let paused = false;

  // Controles
  card.appendChild(slider('Longitud (m)', 0.5, 5, 0.1, L, v => L = v));
  card.appendChild(slider('Gravedad (m/s²)', 0, 20, 0.1, G, v => G = v));
  card.appendChild(slider('Amortiguamiento', 0, 0.2, 0.01, b, v => b = v));
  card.appendChild(slider('Ángulo inicial (º)', 0, 90, 1, theta*180/Math.PI, v => {
  ...
  function draw() {
    ...
    g.fillText(`Periodo teórico ≈ ${(2*Math.PI*Math.sqrt(L/G)).toFixed(2)} s`, 20, 40);
  }
